// app/api/widget/[widgetId]/route.ts - VERSION VANILLA COMPATIBLE NEXT.JS 15
...
export async function GET(
  ...
  try {
    ...
    // 🎯 HTML VANILLA - COPIE EXACTE DE TON DASHBOARD
    const html = `
<!DOCTYPE html>
<html lang="fr">
<head>
  ...
  <!-- ✅ TAILWIND CSS - EXACTEMENT COMME TON DASHBOARD -->
  ...
  <style>
    * {
    ...
    /* Configuration Tailwind pour être identique à ton dashboard */
    .chat-widget {
...
<body>
  ...
  <script>
    ...
    // 🎯 WIDGET QUI COPIE EXACTEMENT TON DASHBOARD
    class DashboardCloneWidget {
      constructor(config) {
        this.config = config;
        this.messages = [];
        this.isOpen = false;
      ...
      createWidget() {
        ...
        // 🎯 HTML STRUCTURE IDENTIQUE À TON DASHBOARD
        root.innerHTML = \`
          <div class="chat-widget" style="--primary-color: \${this.config.primaryColor};">
            ...
            <!-- CHAT BUTTON -->
            <button id="chat-button" class="chat-button animate-bounce-in" style="background-color: \${this.config.primaryColor};" aria-label="Ouvrir le chat">
            ...
            <!-- CHAT WINDOW -->
            <div id="chat-window" class="chat-window animate-expand-from-button \${isDark ? 'dark' : ''}" style="width: \${this.config.width}px; height: \${this.config.height}px; --primary-color: \${this.config.primaryColor}; display: none;">
              
              <!-- HEADER -->
              <div class="chat-header">
                <div class="chat-header-content">
                  <div class="chat-avatar-container">
                    <img src="\${this.config.avatar || '/Default Avatar.png'}" alt="Assistant Avatar" class="chat-avatar" onerror="this.src='/Default Avatar.png'"/>
                ...
                <div class="chat-actions">
                  <button id="reset-btn" class="chat-action-btn" title="Nouvelle conversation" aria-label="Nouvelle conversation">
                  ...
                  <button id="close-btn" class="chat-action-btn" title="Fermer" aria-label="Fermer le chat">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <!-- MESSAGES AREA -->
              <div id="messages-area" class="chat-messages \${isDark ? 'dark' : ''} custom-scrollbar">
              ...
              <div class="chat-input-area \${isDark ? 'dark' : ''} animate-slide-up">
                <div class="chat-input-container">
                  ...
                  />
                  <button id="send-btn" class="chat-send-btn animate-button-hover" style="background-color: \${this.config.primaryColor};" aria-label="Envoyer le message">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      async sendMessage() {
        ...
        try {
          ...
          const response = await fetch(\`\${window.BASE_URL}/api/agents/\${this.config.selectedAgent}/ask\`, {
            method: 'POST',
            headers: {
      ...
      renderMessages() {
        ...
        // 🎯 STRUCTURE IDENTIQUE À TON DASHBOARD AVEC TAILWIND
    ...
    document.addEventListener('DOMContentLoaded', function() {
      window.chatWidget = new DashboardCloneWidget(window.WIDGET_CONFIG);
    });
